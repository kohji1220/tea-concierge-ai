        ...
        if not checker_url and not checker_text and not checker_image:
            ...
            if not combined_text.strip() and checker_image is None:
                ...
                with st.spinner("RAGデータベースと照合し、科学的・市場的観点から査定中..."):
                    ...
                    * ★重要: もしRAGデータ内に該当する相場情報がなく判断が難しい場合は、むやみにサクラ度を上げず、LLMが持つ一般的な相場観から著しい乖離がないかだけをチェックし、判断を保留（加点も減点もしない）してください。

                    ### ③ 「ごまかし」と「不誠実さ」の検知（サクラ度を急上昇させる要素）
                    * 情緒的ワードの多用（目安: +5〜+15点）: 「奇跡の」「至福の」「幻の」といった、味の事実とは無関係な過剰な煽りや感情論のみをペナルティ対象とします。「上品なコク」「甘み」「モルティーな香り」「なめらか」といった、一般消費者向けのお茶のテイスティング表現（味や香りの説明）は許容し、絶対にペナルティ（減点）対象にしないでください。
                    * 健康効果の過剰・違法な主張（目安: +25〜+30点）: 薬機法に抵触する表現。
                    * 煽り文句（目安: +10〜+15点）: 「ランキング1位！」「今だけ半額！」などの過剰な煽り。
                    * 不自然なレビュー（目安: +15〜+25点）
                    * 販売者の透明性欠如（目安: +10〜+20点）
                    ...
                    {{
                      ...
                      "analysis_details": {{
                        ...
                        "price_consistency": "[②価格と主張の整合性に関するRAGデータとの比較分析。RAGにデータがない場合はその旨を記載し、一般常識から推測]",
                        "deception_signals": "[③ごまかしシグナルの検知結果。お茶の味の表現は許容すること]"
                      }},
